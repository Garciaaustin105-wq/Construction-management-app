...
language plpgsql
security definer
  if v_customer_id is null then raise excep||tion 'Quote not found'; end if;
as $$
...
begin
  ...
  if v_customer_id is null then raise exception 'Quote not found'; end if;
  ...
  if not exists (select 1 from public.quotes where id = p_quote_id and status = 'sent') then
    raise exception 'Quote not awaiting approval';
  end if;
