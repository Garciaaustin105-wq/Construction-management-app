  ...
  return (
    <section className="space-y-4">
      ...
      {loading ? (
        ...
        // Desktop UI pass, phase 2: mobile rows moved verbatim into mobileCard
        // (bare, single divided container preserved via mobileListClassName);
        // the lg: grid-row table became the shared dense table, with the
        // inline edit form rendered through rowExpansion.
        <DataTable
          columns={[
            {
              key: "name",
              ...
              cell: (m) => (
                <span className="min-w-0 block">
                  <span className="block font-medium text-gray-900 truncate max-w-56">{m.name}</span>
                  <span className="block text-xs text-gray-400 truncate">
                    {!!m.user_id ? "App user" : "Scheduling only"}
                  </span>
                </span>
              ),
            },
            {
              ...
              header: "Contact",
              cell: (m) => (
                <span className="text-sm text-gray-600 truncate block max-w-56">
                  {[m.trade, m.phone].filter(Boolean).join(" · ") || "—"}
                </span>
              ),
              hideOnMobile: true,
            },
            {
              ...
              cell: (m) =>
                m.applicator_license_number ? (
                  <span className="text-sm text-gray-600 truncate block max-w-64">
                    {m.applicator_license_number}
          ...
          rows={filtered}
          framed
          mobileListClassName="bg-white rounded-lg shadow-sm divide-y divide-gray-100 lg:hidden"
          mobileCardBare
          mobileCardClassName="p-3 flex items-center gap-3"
          rowExpansion={(m) =>
            editId === m.id ? (
