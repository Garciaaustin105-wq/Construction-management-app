...
// Office send-history panel for an estimate (Issue 3). Lists every archived
// `estimate_sends` row — each is an immutable snapshot of exactly what the
...
export default function EstimateSendHistory({
  estimateId,
}: {
  estimateId: string;
}) {
      ...
      {sends.length === 0 ? (
        ...
        sends.map(({ row, senderName }) => (
          ...
          >
            ...
            <Link
              href={`/estimates/${estimateId}/sends/${row.id}`}
