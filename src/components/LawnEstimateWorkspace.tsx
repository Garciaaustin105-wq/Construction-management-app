...
export default function LawnEstimateWorkspace({
  ...
  const lineItemsSection = (
    ...
    <div className="space-y-2 rounded border border-gray-200 bg-gray-50 p-2">
      ...
      {/* The second measurement source. Gated on `editable` for the same reason
          the pricing controls are: importing WRITES areas, and a sent estimate
          must not move. It sits beside the document link rather than in the
          map's toolbar because an import is a deliberate office action, not
          something done mid-draw. */}
      {editable && (
        <Link
          href={`/lawn/estimate/${estimate.id}/import`}
          className="flex items-center justify-center gap-2 rounded-lg bg-white px-3 py-2 text-xs font-semibold text-violet-700 shadow-sm hover:bg-gray-50"
        >
          <Upload className="h-3.5 w-3.5" />
          Import a field measurement
        </Link>
      )}
    </div>
