...
import { Plus, ArrowLeft, ListChecks, Loader2, Layers, Search, Archive, ArchiveRestore, Trash2 } from "lucide-react";
...
// Sort headers are the shared SortHeader (desktop UI pass); the page keeps
// only the sort STATE.
        ...
        {rows.length === 0 ? (
          ...
          <>
            ...
            {displayRows.length === 0 ? (
              ...
              // mobileCard (bare mode — the card carries its own shell and
              // its own row Link, so DataTable adds neither chrome nor a
              // wrapper link); the lg: grid-row table became the shared dense
              // table, with rowHref taking over the per-cell links.
              <DataTable
                columns={[
                  {
                    key: "sel",
                  ...
                  {
                    key: "name",
                    header: <SortHeader col="name" label="Job / Customer" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />,
                    cell: ({ jobName, custName }) => (
                      <span className="min-w-0 block">
                        <span className="font-medium text-gray-900 truncate block max-w-72">{jobName}</span>
                        {custName && <span className="text-xs text-gray-500 truncate block max-w-72">{custName}</span>}
                      </span>
                    ),
                  },
                  {
                    key: "service",
                    header: <SortHeader col="service" label="Service" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />,
                    cell: ({ service }) => <span className="text-sm text-gray-600 truncate block max-w-40">{service}</span>,
                  },
                  {
                    key: "crew",
                    header: <SortHeader col="crew" label="Crew" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />,
                    cell: ({ crewNames }) => <span className="text-sm text-gray-500 truncate block max-w-64">{crewNames || "Unassigned"}</span>,
                  },
                  {
                    key: "status",
                    header: <SortHeader col="status" label="Status" sortKey={sortKey} sortDir={sortDir} onSort={toggleSort} />,
                    cell: ({ row: s }) =>
                      s.jobs?.archived_at != null ? (
                ...
                rows={displayRows}
                rowHref={(d) => `/lawn/schedules/${d.row.id}`}
                framed
                mobileCardBare
                mobileCardClassName="bg-white rounded-lg shadow-sm"
                mobileCard={({ row: s, jobName, custName, service, crewNames }) => {
                  const archived = s.jobs?.archived_at != null;
                  const busy = rowBusy === s.id;
                  return (
                    <>
