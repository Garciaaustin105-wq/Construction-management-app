...
function DailyLogForm() {
  ...
  return (
    <PageContainer
      title="New Daily Log"
      backHref={preselectedJob ? `/jobs/${preselectedJob}` : "/daily-logs"}
      backLabel={preselectedJob ? "Back to job" : "Daily Logs"}
      maxWidth="form"
    >
        <form onSubmit={handleSubmit}>
          <FormGrid columns={2}>
            ...
            <textarea value={workPerformed} onChange={e => setWorkPerformed(e.target.value)} rows={3} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg text-base lg:col-span-2" placeholder="Work performed" />
            ...
            <input type="file" accept="image/*" onChange={e => setPhotoFile(e.target.files?.[0] || null)} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-lg text-base lg:col-span-2" />
              <Button
                type="submit"
                disabled={loading}
                size="lg"
