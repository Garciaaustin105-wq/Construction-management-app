...
// Job status -> badge tone (was the legacy StatusBadge's shared palette).
...
export default async function CustomerPortal() {
  ...
  return (
    <PageContainer title={profile?.full_name ?? "Project Portal"} subtitle={me.user.email ?? ""} showSignOut maxWidth="list">
      <ClientPullToRefresh>
        {/* Financial summary */}
        {hasSummary && (
          <section>
            ...
            >
              <div className="p-4">
                <div className="flex justify-between items-start mb-1">
                  ...
                  <StatusBadge tone={JOB_STATUS_TONE[job.status] ?? "neutral"}>{job.status.replace("_", " ")}</StatusBadge>
                </div>
                {job.address && (
