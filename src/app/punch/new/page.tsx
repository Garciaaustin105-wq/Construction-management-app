...
function PunchForm() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: crewList } = await supabase.from("profiles").select("id, full_name").in("role", ["crew", "superintendent"]).order("full_name");
      setCrew(crewList ?? []);
    })();
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24 lg:pb-10">
      <header className="sticky top-0 z-40 bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <h1 className="text-lg font-bold text-gray-900 absolute left-1/2 -translate-x-1/2">New Punch Item</h1>
        <div className="w-16" />
      </header>
